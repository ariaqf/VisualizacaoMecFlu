...
double DUV(double LL, double L, double M, double R, double RR, double KL, double KR, 
            double DD, double DM, double D, double DP, double PP, double alpha, bool border, bool debug=false) {
    ...
    double UL=(KL>=0) ? 0:1;
      ...
      {// Sharp and Monotonic Algorithm for Realistic Transport (SMART) 2nd-Order
         ...
         Phi_L=(1-UL)*(L-LL)/(((M-LL)!=0.0)?(M-LL):1E-15) + UL*(M-R)/(((L-R)!=0.0)?(L-R):1E-15);
	      if(debug) {
            std::cout << "PHIR: " << Phi_R << std::endl;
            std::cout << "PHIL: " << Phi_L << std::endl;
         }
         if((Phi_R>=0) && (Phi_R<(3./74))){
            if(debug)
               std::cout << "Phi_R >= 0" << std::endl;
            UR=(1-UR)*(10*M-9*L)+UR*(10*R-9*RR);
            if(debug) {
               std::cout << "UR: " << UR << std::endl;
         ...
         if((Phi_L>=0) && (Phi_L<(3./74))) {
            ...
            if((Phi_L>=(3.0/74)) && (Phi_L<(5.0/6))){
               UL=(1-UL)*((3.0*M+6.*L-LL)/8.0) + UL*((3.0*L+6.0*M-R)/8.0);
               // UL=(1-UL)*((3./8)*M+(6./8)*L-(1./8)*LL)+UL*((3./8)*L+(6./8)*M-(1./8)*R);
            }
            else {
               if((Phi_L>=(5./6)) && (Phi_L<=1)){
